...
namespace AsteroidBeltAssault {
    class AsteroidManager {
        ...
        public void AddAsteroid() {
            ...
            }//Selecting asteroids from spriteSheet
            newAsteroid.Rotation = MathHelper.ToRadians((float)rand.Next(0, 360));
            newAsteroid.CollisionRadius = 15;
            Asteroids.Add(newAsteroid); //Adding Rectangles to the Asteroids List
        }
        public void Clear() {
            Asteroids.Clear();
        }
        public AsteroidManager(int asteroidCount, Texture2D texture,
            Rectangle initialFrame, int asteroidFrames,
            ...
            do {
                ...
                foreach (Sprite asteroid in Asteroids) {
                    if (asteroid.IsBoxColliding(new Rectangle((int)location.X,
                        (int)location.Y, initialFrame.Width, initialFrame.Height))) {
                        locationOK = false;
                    }
                }
                tryCount++;
        ...
        private bool isOnScreen(Sprite asteroid) {
            if (asteroid.Destination.Intersects(new Rectangle(
               -screenPadding, -screenPadding, 
               screenWidth + 2 * screenPadding, screenHeight + 2 * screenPadding))) {
                return true;
            }else {
                return false;
            }
